$(document).ready(function () {
  ...
  function validateFormData(formData, isEdit = false) {
    ...
    if (!matriculeRegex.test(formData.matricule)) {
      showNotification(
        "error",
        "Le matricule doit être au format 1 à 4 chiffres, TUN, 4 chiffres (ex: 1234TUN5678)."
      );
      return false;
    }
    if (!statusRegex.test(formData.status)) {
      showNotification(
        "error",
        "Le statut doit contenir uniquement des lettres."
      );
      return false;
    }
    if (isNaN(formData.nbPlace) || formData.nbPlace < 0) {
      showNotification(
        "error",
        "Le nombre de places doit être un nombre positif ou zéro."
      );
      return false;
    }
    if (isNaN(formData.cylinder) || formData.cylinder < 0) {
      showNotification(
        "error",
        "Le cylindre doit être un nombre positif ou zéro."
      );
      return false;
    }
    return true;
        ...
        if (response && response.status === "success" && response.data) {
          try {
            var newRow = table.row
              ...
              .draw(false)
              .node(); // Partial draw to prevent freeze
